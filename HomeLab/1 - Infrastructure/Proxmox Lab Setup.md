...
**A8 rollback copies:** kept powered off, `tag=20` restored on both (`qm set 100/101 --net1 ...,tag=20`), as an emergency fallback. **Never run an A8 copy and a lab copy of the same VM at the same time** — DC01 especially, since two live copies of a domain controller risks a USN rollback.
