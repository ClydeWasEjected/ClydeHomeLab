...
```mermaid
flowchart LR
    isp["ISP router"] --> pf["pfSense<br/>LAN 10.10.10.1/24"] --> ap["AP"] --> home["home network"]
```
...
> [!CAUTION] Not verified closed
> The pfSense web GUI may still be reachable from the WAN side. Check before trusting this is closed.
...
| Attempt | Result |
|---|---|
| Add LAN subnet to the DNS Resolver access list | ❌ still failing |
| Enable **Forward Mode** on the DNS Resolver | ✅ fixed |
